...
class PopupRoleView: UIView {
    ...
    func showViewWithRole(){
        ...
        case .worker:
            ...
            typeTextLabel.attributedText = self.changeTextColor(text: "我是\(mRoleType.rawValue)", color: UIColorHex("00b2ff"), range: NSRange(location: 2, length: 2))
             infoTextLabel.text = "成熟干练/收入稳定"
            break
        case .student:
            roleImageView.image = UIImage(named: "pop_role_student_125x130")
            typeTextLabel.attributedText = self.changeTextColor(text: "我是\(mRoleType.rawValue)", color: UIColorHex("f9db55"), range: NSRange(location: 2, length: 2))
            infoTextLabel.text = "朝气蓬勃/勤奋好学"
            break
        case .freedom:
            roleImageView.image = UIImage(named: "pop_role_free_125x130")
            typeTextLabel.attributedText = self.changeTextColor(text: "我是\(mRoleType.rawValue)", color: UIColorHex("e9342d"), range: NSRange(location: 2, length: 3))
            infoTextLabel.text = "无拘无束/悠闲自得"
            break
        }
    }
    ...
    //MARK: - Action
    var onClickSelectBtn : ((_ roleType: RoleType)->())?
    ...
    func selectBtnAction(){
        ...
        if let onClickSelectBtn = onClickSelectBtn{
            onClickSelectBtn(self.mRoleType)
        }
    }
}
